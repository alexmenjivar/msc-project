...
df = pd.read_csv("data/Breast.csv", na_values=["?"], low_memory=False)
y = df["class"].values
X = df.drop(columns=["class"]).values.astype(float)   # gaps become NaN
print(f"Breast: {X.shape[0]} samples, {X.shape[1]} genes")
print(f"Missing cells: {np.isnan(X).sum()}, classes={dict(pd.Series(y).value_counts())}\n")
...
def select_genes(Xtr, ytr, seed=42):
    ...
    return fidx[hc_mask==1], imp   # return imputer too, to reuse on test
